...
final class APIManagerViewController: UITableViewController {
    ...
    @objc func infoButtonAction(_ button: UIButton) -> Void {
        ...
        api.delegate = self;
        api.weather(of: "北京", policy: self.policy)
        api.weather(of: "上海", policy: self.policy)
        api.weather(of: "武汉", policy: self.policy)
        api.weather(of: "深圳", policy: self.policy)
    }
    
    var policy = APIConcurrencyPolicy.default
...
extension APIManagerViewController: APIConcurrencyPolicyViewControllerDelegate {
    ...
    func viewController(_ viewController: APIConcurrencyPolicyViewController, didSelect policy: APIConcurrencyPolicy) {
        self.policy = policy
        self.dataArray.removeAll()
        self.tableView.reloadData()
    }
